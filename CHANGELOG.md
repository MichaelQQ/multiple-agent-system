...
- **Failure history on task detail page** (`/task/<id>`): the web UI now renders a **Failure history** section that displays patterns from `patterns.jsonl` matching the task's goal (via `goal_similarity` threshold). Each entry shows `goal_sample`, `terminal_reason`, `count`, `last_seen`, clickable links to related task IDs, and expandable rejected attempt snippets. The `?failure_filter=blocking` query parameter restricts the view to patterns with terminal reasons (`revision_cycles_exceeded`, `max_retries_exceeded`, `convergence_detected`). Implemented in `src/mas/web/app.py` (`_task_detail` route) and `src/mas/web/templates/task.html`.

- **Budget forecast warning on dashboard**: computes daily burn rate from audit data and displays a depletion warning on the web dashboard when the budget is projected to exhaust within the configured threshold days (default 7). New config parameter: `forecast_warning_days_threshold` (integer, default `7`, must be ≥ 0). Implemented in `src/mas/cost_helpers.py` (`compute_burn_rate()`, `forecast_exhaustion_days()`) and `src/mas/web/app.py` (`/stats` route).
